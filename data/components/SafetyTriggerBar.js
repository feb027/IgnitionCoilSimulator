...
export function SafetyTriggerBar({ state, sendAction, label = "IGT TRIGGER", is4Pin = false, isLocked: extLocked, onToggleLock: extToggleLock }) {
    ...
    const setLockState = (nextLock) => {
        if (extToggleLock) extToggleLock(nextLock);
        else setLocalLocked(nextLock);
    };

    // Auto-expand when running, auto-collapse when stopped
    useEffect(() => {
        if (isRunning) {
            setIsVisible(true);
        } else {
    ...
    return html`
        ...
        >
            ${!isVisible && !isRunning ? html`
                <!-- COMPACT BOTTOM STRIP (COLLAPSED STATE) -->
                <div 
                    style="padding: 8px 14px; display: flex; justify-content: space-between; align-items: center; cursor: pointer; touch-action: none;"
                    onClick=${handleToggleDock}
                    onTouchStart=${(e) => e.stopPropagation()}
                >
                    <div style="font-size: 0.76rem; font-weight: 800; color: var(--neon-cyan); display: flex; align-items: center; gap: 8px;">
                        <span>⚡ DOCK TRIGGER UTAMA</span>
                        <span style="font-size: 0.65rem; color: #aaa; font-weight: normal; background: rgba(255,255,255,0.06); padding: 1px 6px; border-radius: 3px;">👆 Ketuk Untuk Buka</span>
                    </div>
                    <div style="display: flex; align-items: center; gap: 6px;">
                        <button 
                            class="btn" 
                            style="padding: 3px 8px; font-size: 0.72rem; font-weight: 800; border-color: ${isLocked ? 'var(--neon-orange)' : 'var(--neon-green)'}; background: ${isLocked ? 'rgba(255, 149, 0, 0.15)' : 'rgba(0, 255, 102, 0.15)'}; color: ${isLocked ? 'var(--neon-orange)' : 'var(--neon-green)'}; cursor: pointer;"
                            onClick=${handleToggleLockClick}
                        >
                            ${isLocked ? '🔒 TERKUNCI' : '🔓 BUKA KUNCI'}
                        </button>
                        <button class="btn" style="padding: 3px 8px; font-size: 0.72rem; font-weight: 900; background: var(--neon-cyan); color: #000; border-color: var(--neon-cyan);">
                            ▲ MENU
                        </button>
                    </div>
                </div>
            ` : html`
                <!-- FULL TRIGGER DOCK (EXPANDED STATE) -->
                <div 
                    style="max-width: 1200px; margin: 0 auto; padding: 8px 12px; display: flex; flex-direction: column; gap: 6px; touch-action: none;"
                    onTouchStart=${(e) => e.stopPropagation()}
                >
                    ${showAlert ? html`
                        <div style="background: rgba(255, 45, 85, 0.3); border: 1px solid var(--neon-red); color: #fff; font-size: 0.72rem; font-weight: 900; padding: 4px 8px; border-radius: 4px; text-align: center;" class="pulse-alarm-red">
                            ⚠️ BUKA KUNCI PENGAMAN DAHULU SEBELUM MENEKAN TRIGGER!
                        </div>
                    ` : ''}
